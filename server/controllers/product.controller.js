...
export const getProductsByType = asyncHandler(async (req, res) => {
  ...
  if (section) {
    ...
    // Fallback: If no products are explicitly ordered/saved in this section yet,
    // query by productType array field directly.
    if (products.length === 0) {
      ...
      if (productType.toLowerCase() === "latest") {
...
// Get all secret products
export const getSecretProducts = asyncHandler(async (req, res) => {
  const {
    page = 1,
    limit = 12,
    search = "",
  ...
  const formattedProducts = products.map((product) => {
    ...
    return {
      ...
      image: product.images.find((img) => img.isPrimary)
        ? getFileUrl(product.images.find((img) => img.isPrimary).url)
        : product.images.length > 0
        ? getFileUrl(product.images[0].url)
        : null,
      variants: product.variants.map((variant) => ({
        ...variant,
        images: variant.images
  ...
  res.status(200).json(
    new ApiResponsive(
      ...
      "Secret products fetched successfully"
    )
  );
});

// Get secret product by slug
export const getSecretProductBySlug = asyncHandler(async (req, res) => {
  const { slug } = req.params;
  ...
  if (!product) {
    throw new ApiError(404, "Secret product not found");
  }
  ...
  const formattedProduct = {
    ...
    category: primaryCategory
      ...
      : null,
    subCategories: product.subCategories.map((sc) => ({
      id: sc.subCategory.id,
      name: sc.subCategory.name,
      slug: sc.subCategory.slug,
      image: sc.subCategory.image
        ? getFileUrl(sc.subCategory.image)
        : null,
    })),
    brand: product.brand
      ? {
  ...
  res.status(200).json(
    new ApiResponsive(
      200,
      { product: formattedProduct },
      "Secret product fetched successfully"
    )
  );
});
