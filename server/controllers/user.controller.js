...
export const registerUser = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email,
      subject: "Your OTP for Email Verification - rhoseatte",
...
export const guestCheckoutInit = asyncHandler(async (req, res, next) => {
  ...
  if (existingUser) {
    ...
    const userWithoutSensitive = { ...existingUser };
  ...
  try {
    ...
    // account a moment ago. Sign straight into it instead of surfacing a
    // raw 500 — the account now genuinely exists.
    if (error.code === "P2002") {
      ...
      if (raceWinner) {
        ...
        const userWithoutSensitive = { ...raceWinner };
        delete userWithoutSensitive.password;
        delete userWithoutSensitive.otp;
        delete userWithoutSensitive.checkoutLoginOtp;
...
export const forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email,
      subject: "Reset Your Password - rhoseatte",
...
export const getOrderDetails = asyncHandler(async (req, res, next) => {
  ...
  // Format order for frontend - use snapshot data when available for immutability
  const formattedOrder = {
    ...
    items: order.items.map(item => {
      ...
      return {
        ...
        originalPrice: snapshot?.originalPrice || parseFloat(item.variant?.price || 0),
        salePrice: snapshot?.salePrice,
        paidPrice: snapshot?.paidPrice || parseFloat(item.price),
        variant: item.variant ? {
          ...item.variant,
          // Map attributes for consistent UI
          options: snapshot?.variantAttributes?.length > 0
            ? snapshot.variantAttributes.map(attr => ({
                name: attr.name,
                value: attr.value,
                hexCode: null
              }))
...
export const requestAccountDeletion = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Confirm Account Deletion - rhoseatte",
